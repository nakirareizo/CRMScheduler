...
namespace CRMSync
{
    class SQLHelper : IDisposable
    {
        ...
        internal static SqlConnection GetConnectionMOF()
        {
            SqlConnection output = new SqlConnection(GetConnectionStringMOF());
            output.Open();
            return output;
        }

        private static string GetConnectionStringMOF()
        {
            string output = Properties.Settings.Default.MOFConnStr;
            return output;
        }
    }
}
